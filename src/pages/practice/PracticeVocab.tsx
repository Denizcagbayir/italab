import { useMemo, useState } from 'react';
import { Link } from 'react-router-dom';
import vocab from '../../data/vocab.json';
import { turkishIncludesAny, shuffle } from '../../lib/normalize';
import { SpeakButton } from '../../components/speech/SpeakButton';
import { useProgress } from '../../store/progressStore';

type Item = { it: string; category: string; tr: string[]; note: string };

const ALL = vocab as Item[];

export function PracticeVocab() {
  const ignoreAccents = useProgress((s) => s.settings.ignoreAccents);
  const [n, setN] = useState(12);
  const [session, setSession] = useState(() => shuffle(ALL).slice(0, 12));
  const [answers, setAnswers] = useState<Record<number, string>>({});
  const [revealed, setRevealed] = useState<Record<number, boolean>>({});

  const rebuild = () => {
    setSession(shuffle(ALL).slice(0, n));
    setAnswers({});
    setRevealed({});
  };

  const stats = useMemo(() => {
    let ok = 0;
    let filled = 0;
    session.forEach((item, i) => {
      const a = answers[i]?.trim();
      if (!a) return;
      filled++;
      if (turkishIncludesAny(a, item.tr)) ok++;
    });
    return { ok, filled, rate: filled ? Math.round((ok / filled) * 100) : 0 };
  }, [answers, session, ignoreAccents]);

  return (
    <div className="page practice-drill">
      <Link to="/practice" className="back-link">
        ← Pratik
      </Link>
      <header className="page-header">
        <h1>Kelime</h1>
        <p className="lede">
          İtalyanca ifadeyi oku (istersen dinle), Türkçe karşılığını yaz.
        </p>
      </header>
      <div className="toolbar">
        <label>
          Adet
          <select
            value={n}
            onChange={(e) => setN(Number(e.target.value))}
          >
            {[8, 12, 20, 30].map((x) => (
              <option key={x} value={x}>
                {x}
              </option>
            ))}
          </select>
        </label>
        <button className="btn primary" onClick={rebuild}>
          Yeni set
        </button>
        <span className="pill">
          {stats.ok}/{stats.filled} · %{stats.rate}
        </span>
      </div>
      <div className="drill-table">
        {session.map((item, i) => {
          const a = answers[i] ?? '';
          const filled = a.trim().length > 0;
          const ok = filled && turkishIncludesAny(a, item.tr);
          return (
            <div key={`${item.it}-${i}`} className="drill-row">
              <div className="q">
                <span lang="it">{item.it}</span>
                <SpeakButton text={item.it} small />
                {item.category && <em className="tag">{item.category}</em>}
              </div>
              <input
                value={a}
                onChange={(e) =>
                  setAnswers((prev) => ({ ...prev, [i]: e.target.value }))
                }
                placeholder="Türkçe…"
              />
              <div className="status">
                {filled ? (ok ? '✔' : '✘') : ''}
                <button
                  className="btn ghost tiny"
                  onClick={() =>
                    setRevealed((r) => ({ ...r, [i]: !r[i] }))
                  }
                >
                  {revealed[i] ? 'Gizle' : 'Göster'}
                </button>
              </div>
              {revealed[i] && (
                <p className="reveal">{item.tr.join('; ')}</p>
              )}
            </div>
          );
        })}
      </div>
    </div>
  );
}
